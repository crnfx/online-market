@props([
  'count' => '0',
  'totalSum' => '0'
])
<header class="header">
  <a class="header__logo" href="/">
    <img src="{{ asset('images/Logo.png')  }}" alt="Logo">
  </a>
  <form class="header__search-form" method="GET" action="{{ route('catalog') }}" role="search">
    <label for="header-search" class="visually-hidden">Поиск товаров</label>
    <input
      id="header-search"
      name="search"
      class="header__search-input"
      type="search"
      placeholder="Введите название товара"
      value="{{ request('search') }}"
      aria-label="Поиск товаров"
    >
    <button class="header__search-btn" type="submit" aria-label="Найти">
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        <circle cx="11" cy="11" r="8"></circle>
        <path d="m21 21-4.35-4.35"></path>
      </svg>
    </button>
  </form>

  <ul class="header__list">
    <li class="header__item"><a href="tel:+78001234567">+78001234567</a></li>
    <li class="header__item"><a href="mailto:info@onlinemarket.ru">info@onlinemarket.ru</a></li>
  </ul>

  <a class="header__cart--link" href="{{ route('cart') }}">
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        d="M0 1.5C0 1.22386 0.223858 1 0.5 1H2C2.22943 1 2.42943 1.15615 2.48507 1.37873L2.89039 3H14.5C14.6498 3 14.7917 3.06716 14.8867 3.18301C14.9816 3.29885 15.0197 3.45117 14.9903 3.59806L13.9903 8.59806C13.9454 8.82257 13.7537 8.9879 13.525 8.99937L4.12822 9.47069L4.41496 11H13C13.2761 11 13.5 11.2239 13.5 11.5C13.5 11.7761 13.2761 12 13 12H12H5H4C3.75939 12 3.55291 11.8286 3.50856 11.5921L2.01131 3.6068L1.60961 2H0.5C0.223858 2 0 1.77614 0 1.5ZM3.10246 4L3.94223 8.47876L13.0861 8.02013L13.8901 4H3.10246ZM5 12C3.89543 12 3 12.8954 3 14C3 15.1046 3.89543 16 5 16C6.10457 16 7 15.1046 7 14C7 12.8954 6.10457 12 5 12ZM12 12C10.8954 12 10 12.8954 10 14C10 15.1046 10.8954 16 12 16C13.1046 16 14 15.1046 14 14C14 12.8954 13.1046 12 12 12ZM5 13C5.55228 13 6 13.4477 6 14C6 14.5523 5.55228 15 5 15C4.44772 15 4 14.5523 4 14C4 13.4477 4.44772 13 5 13ZM12 13C12.5523 13 13 13.4477 13 14C13 14.5523 12.5523 15 12 15C11.4477 15 11 14.5523 11 14C11 13.4477 11.4477 13 12 13Z"
        fill="black"/>
    </svg>
    <div class="header__cart--wrapper">
      <span>Товаров в корзине: {{ $count }}</span>
      <span>Сумма: {{ $totalSum  }}</span>
    </div>
  </a>
  <button class="header__menu" type="button" aria-label="Открыть меню" aria-expanded="false">
    <span></span>
    <span></span>
    <span></span>
  </button>
  <ul class="header__list header__list--menu">
    <li class="header__item"><a class="header__link" href="{{ route('home')  }}">Главная</a></li>
    <li class="header__item"><a class="header__link" href="{{ route('catalog')  }}">Каталог</a></li>
    <li class="header__item"><a class="header__link" href="{{ route('service') }}">Услуги</a></li>
    <li class="header__item"><a class="header__link" href="#">Контакты</a></li>
  </ul>
</header>
